...
def extract_dicom_axis_patient(src_dir):
    ...
    dir_path = settings.DICOM_SRC_DIR + src_dir + "/"
    patient_id = src_dir
    slices = dicom_basic_process.load_patient(dir_path)
    ...
    pixels = dicom_basic_process.get_pixels_hu(slices)
    image = pixels
    ...
    coord_zxyt = step1_shift_save_ct.get_zxy(patient_id)
    if coord_zxyt:
        for coord_zxy in coord_zxyt:
            ...
            if not invert_order:
                ...
                coord_zxy[0] = step1_shift_save_ct.filter_one_coord(coord_zxy[0], patient_id)
            # print(coord_zxy)
            # 保存用于训练用的（z,x,y）
...
def extract_train_csv(clean_targetdir_first=False, only_patient_id=None):
    ...
    target_dir = settings.TRAIN_LABEL
    ...
    if only_patient_id is None:
        # CT 文件夹
        dirs = step1_shift_save_ct.get_usecoord_patient()
        print('dirs:', dirs)
        if dirs is not None:
            for ct_dir in dirs:
                txt_to_csv(ct_dir)
...
def extract_dicom_axis(clean_targetdir_first=False, only_patient_id=None):
    print("Extracting noudle axis and type for train")
    # CT noudle坐标和类型存储目录:D:/Mywork/coord/
    target_dir = settings.TRAIN_COORD
    # 如果target_dir目录下存在文件，则进入if,删除该目录下所有文件
    if clean_targetdir_first and only_patient_id is None:
...
def extract_dicom_images_patient(src_dir):
    ...
    image = dicom_basic_process.rescale_patient_images(image, pixel_spacing, settings.TARGET_VOXEL_MM, verbose=False)
    ...
    for i in range(image.shape[0]):  # image:(873,500,500)
        ...
        img, mask = dicom_basic_process.get_segmented_lungs(org_img.copy())
        org_img = dicom_basic_process.normalize_hu(org_img)
...
def extract_dicom_images(clean_targetdir_first=False, only_patient_id=None):
    ...
    if only_patient_id is None:
        # CT 文件夹
        dirs = os.listdir(settings.DICOM_SRC_DIR)
        if dirs is not None:
            # # 创建进程池
...
def txt_to_csv(patientID):
    ...
    df_annos.to_csv(settings.TRAIN_LABEL + patientID + "_annotations.csv", index=False, encoding='UTF-8')
