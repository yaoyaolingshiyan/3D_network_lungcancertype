...
def get_use_coord_list():
    all_lines = []
    coord_dir = 'D:/zmy_data/use_coord.txt'
    f = open(coord_dir, 'rb')
    line1 = f.readline().decode('UTF-8')

    while line1:
        patient = str(line1.split(':')[0])
        coord_z = str(line1.split(':')[1])
        coord_x = str(line1.split(':')[2])
        coord_y = str(line1.split(':')[3])
        lung_type = str(line1.split(':')[4])
        lung_r = int(line1.split(':')[5].replace('\r', '').replace('\n', ''))
...
def look_tag_ct_xy():
    tag_path = 'D:/zmy_data/tag_img/'
    ct_path = 'D:/zmy_data/verify_img/'
            ...
            if jpg_id == use_info[0] and jpg_z == use_info[3]:
                ...
                while (1):
                    ...
                    # 保存病灶坐标和半径
                        ...
                        with open('D:/zmy_data/generate_txt/sign_label.txt', "a",
                    ...
                    # 跳过这张图像，记录这张图像编号
                        ...
                        print(line)
                        with open('D:/zmy_data/generate_txt/bad_id.txt', "a",
                                  encoding="UTF-8") as target:
                            target.write(line + '\n')
                        break
                cv2.destroyAllWindows()
